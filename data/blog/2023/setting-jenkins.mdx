---
title: 'Jenkins를 이용해 CI/CD 구축하기'
date: 2023-07-10T10:01:01Z
lastmod: '2023-05-13'
tags: ['java', 'jenkins', 'CI/DC', 'Spring', 'Gitlab']
draft: false
summary: 'Jenkins를 이용해 Spring Boot코드를 서버에 자동 배포하는 환경을 구축'
layout: PostSimple
bibliography: references-data.bib
---

개발 서버에 코드를 배포하기 위해 여러가지 체크를 해보았고 무엇을 사용할지 고민하다 Jenkins를 이용해 push방식으로 배포하기로 결정했고 이를 정리해 보았습니다.

# 왜?

현재 개발서버는 각각 서버에 Docker가 기동중이며 해당 docker가 있는 서버에 ftp로 파일을 올리고 docker load 명령어를 통해 이미지를 로드하고 docker run 명령어를 통해 기동하는 방식으로 배포를 하고 있습니다.

이 방식은 간단하고 빠르게 배포가 가능하지만 당연하게도 수동으로 하고 있어서 불편한 점이 많았습니다.

문제점은 아래와 같았습니다.

1. 배포를 하기 위해 각 서버에 접속해야함
2. 배포할 서버가 많아지면 배포하는데 시간이 오래걸림

이런 문제점을 해결하고자 Jenkins를 이용해 CI/CD를 구축하기로 결정했습니다.
...
아래 명령어를 실행시켜 Jenkins LTS(Long Term Support) 버전의 이미지를 다운로드 합니다.
...
```java
package com.br.indiefit.gamerelease.domain;
...
그리고 아래와 같이 해당 클래스의 인스턴스를 다음과 같이 사용하곤 했습니다.

```java
Category category = CategoryEntity.builder()
                .title("액션")
                .creatorId(userId)
                .build();
...
category.setTitle("롤플레잉");
```

이렇게 @Build를 사용하는것이 옳을까요?

## 아뇨 옳지 않습니다

빌더 패턴은 변경할 수 없는 객체를 생성하는 패턴이며 인스턴스화 되면 절대 수정하면 안됩니다.

하지만 객체가 만약에 변경이 가능해야하는 객체라면 어떻게 해야할까요?
